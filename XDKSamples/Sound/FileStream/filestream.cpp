...
HRESULT CWaveFileStream::Initialize( const CHAR* strFileName, DWORD * pdwPercentCompleted )
{
    ...
    assert( WAVE_FORMAT_XBOX_ADPCM == pwfxSourceFormat->wFormatTag );
    ...
    if( g_bDoDebugChecks )
    {
        ...
        assert( !xmi.dwInputSize );
        assert( !(FILESTRM_PACKET_BYTES % xmi.dwOutputSize) );

        hr = m_pRenderFilter->GetInfo( &xmi );
        if( FAILED(hr) )
            return hr;
        assert( xmi.dwFlags == (XMO_STREAMF_FIXED_SAMPLE_SIZE | XMO_STREAMF_INPUT_ASYNC) );
        assert( FILESTRM_PACKET_BYTES * FILESTRM_PACKET_COUNT >= xmi.dwMaxLookahead );
        assert( !(FILESTRM_PACKET_BYTES % xmi.dwInputSize) );
...
//-----------------------------------------------------------------------------
VOID CWaveFileStream::Pause( DWORD dwPause )
{
...
DWORD WINAPI WAVFileStreamThreadProc( VOID* pParameter )
{
    ...
    CWaveFileStream* pThis = (CWaveFileStream*)pParameter;
