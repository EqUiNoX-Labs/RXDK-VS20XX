...
//-----------------------------------------------------------------------------
HRESULT CCarMesh::RenderMesh( XBMESH_DATA* pMesh, DWORD dwFlags )
{
    ...
    for( DWORD i = dwStartShape; i < dwEndShape; i++ )
    {
        ...
        if( bRender )
        {
            ...
            if( bRenderSubset )
            {
                D3DDevice::DrawIndexedVertices( dwPrimType, pSubsets[i].dwIndexCount,
                                                D3D__IndexData + pSubsets[i].dwIndexStart );
            }
        }
    }
...
class CXBoxSample : public CXBApplication
{
    ...
    CXBMesh            m_Teapot;               // Test Teapot object
    LPDIRECT3DTEXTURE8 m_pSphereMapTexture;    // Texture for Sphere mapping

    CCarMesh           m_Car;                  // Test Car object

    CXBMesh            m_Earth;                // Test Earth Globe object
    ...
    DrawObject         m_DrawObject;           // Which object to draw.
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( m_Earth.Create( "Models\\GEarth.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    // Load the textures
    m_pSphereMapTexture  = m_xprResource.GetTexture( "Scene1" );
    m_pEarthMapTexture   = m_xprResource.GetTexture( "Earth" );
    m_pEarthCloudTexture = m_xprResource.GetTexture( "Clouds" );
    ...
    m_vLookPosition.x = 0.0f;
    m_vLookPosition.y = 0.0f;
    m_vLookPosition.z = 0.0f;

    m_vCameraPosition.x = 5.0f;
    m_vCameraPosition.y = m_vCameraPosition.z = 0.0f;

    // Set initial rotation of object
    m_fXObjRotate = m_fYObjRotate = m_fLightRotate = 0.0f;
    ...
    // Create vertex shaders
    {
        ...
        if( FAILED( XBUtil_CreateVertexShader( "Shaders\\fresnel3.xvu",
                                               dwDecl, &m_dwFresnelShader ) ) )
            return XBAPPERR_MEDIANOTFOUND;

        if( FAILED( XBUtil_CreateVertexShader( "Shaders\\fresnel4.xvu",
                                               dwDecl, &m_dwFresnelEarthShader ) ) )
            return XBAPPERR_MEDIANOTFOUND;

        if( FAILED( XBUtil_CreateVertexShader( "Shaders\\planet.xvu",
                                               dwDecl, &m_dwEarthShader ) ) )
            return XBAPPERR_MEDIANOTFOUND;

        if( FAILED( XBUtil_CreateVertexShader( "Shaders\\car.xvu",
...
HRESULT CXBoxSample::Render()
{
    ...
    RenderGradientBackground( 0xff4040c0, 0xff000000 );
    ...
    switch( m_DrawObject )
    {
        ...
        case Draw_EnvCar:
        {
            ...
            m_Car.RenderMesh( m_Car.GetMesh(0),
        ...
        case Draw_Earth:
        {
            ...
            m_Earth.Render( XBMESH_NOTEXTURES | XBMESH_NOMATERIALS | XBMESH_NOFVF );
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"FresnelReflect" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        if( m_DrawObject != Draw_Earth )
        {
            m_Font.DrawText( 64,  70, 0xffffffff, L"Fresnel Reflectivity" );
            m_Font.DrawText( 64, 100, 0xffffffff, L"Using Shader Fresnel3" );
        }
        else
        {
            m_Font.DrawText( 64,  70, 0xffffffff, L"Fresnel Transparency" );
            m_Font.DrawText( 64, 100, 0xffffffff, L"Using Shader Fresnel4" );
        }
        m_Font.End();
    }
