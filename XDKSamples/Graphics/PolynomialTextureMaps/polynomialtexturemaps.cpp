...
//       Xbox GPU.
//
//       For a full description of polynomial texture maps see 
...
//-----------------------------------------------------------------------------
// Data defining the quad the polynomial texture is applied to.
//-----------------------------------------------------------------------------
struct QUADVERT
{
    D3DVECTOR Pos;
...
class CXBoxSample : public CXBApplication
{
    ...
    IDirect3DTexture8* m_pCoeff1Texture;     // a0, a1, a2 coeffecients texture
    IDirect3DTexture8* m_pCoeff2Texture;     // a3, a4, a5 coeffecients texture
    IDirect3DTexture8* m_pColorTexture;      // Color texture
...
//-----------------------------------------------------------------------------
// Name: Initialize()
// Desc: Load the resource, create the shaders, and initialize the view.
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( XBUtil_CreateVertexShader( "Shaders\\PTM.xvu", vdecl, &m_dwPTMVertexShader ) ) )
        return E_FAIL;

    // Create the pixel shader.
    if( FAILED( XBUtil_CreatePixelShader( "Shaders\\PTM.xpu", &m_dwPTMPixelShader ) ) )
        return E_FAIL;
    ...
    // Get the textures.
    m_pCoeff1Texture = m_xprResource.GetTexture( "MoonPoly1" );
    m_pCoeff2Texture = m_xprResource.GetTexture( "MoonPoly2" );
    m_pColorTexture  = m_xprResource.GetTexture( "MoonColor" );
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    // Place limits so we don't go around the back of the plane.
    if( D3DXVec3Dot( &vTemp, &vNormal ) > 0.0f )
        m_vLight = vTemp;
...
//       the polynomial texture map on it.
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::Render()
{
    // Clear the viewport, zbuffer, and stencil buffer
    m_pd3dDevice->Clear( 0L, NULL, D3DCLEAR_TARGET, 0x00000000, 1.0f, 0 );
    ...
    m_pd3dDevice->SetRenderState( D3DRS_SWATHWIDTH,       D3DSWATH_16 );
    ...
    D3DXVECTOR4 c0;
    c0.x = (float)m_iBias[0] / 255.0f;
    c0.y = (float)m_iBias[1] / 255.0f;
    c0.z = (float)m_iBias[2] / 255.0f;
    m_pd3dDevice->SetPixelShaderConstant( 0, &c0, 1 );

    D3DXVECTOR4 c1;
    c1.x = (float)m_iBias[3] / 255.0f;
    c1.y = (float)m_iBias[4] / 255.0f;
    c1.z = (float)m_iBias[5] / 255.0f;
    m_pd3dDevice->SetPixelShaderConstant( 1, &c1, 1 );

    D3DXVECTOR4 c2;
    c2.x = m_vLight.x * m_vLight.x * 0.5f + 0.5f;
    c2.y = m_vLight.y * m_vLight.y * 0.5f + 0.5f;
    c2.z = m_vLight.x * m_vLight.y * 0.5f + 0.5f;
    m_pd3dDevice->SetPixelShaderConstant( 2, &c2, 1 );

    D3DXVECTOR4 c3;
    c3.x = m_vLight.x * 0.5f + 0.5f;
    c3.y = m_vLight.y * 0.5f + 0.5f;
    c3.z = 1.0f;
    m_pd3dDevice->SetPixelShaderConstant( 3, &c3, 1 );
    ...
    m_pd3dDevice->SetTextureStageState( 2, D3DTSS_ALPHAKILL, D3DTALPHAKILL_ENABLE );
    ...
    m_pd3dDevice->SetTextureStageState( 2, D3DTSS_ALPHAKILL, D3DTALPHAKILL_DISABLE );
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"PolynomialTextureMaps" );
...
//       sample textures.  Also saved out the textures, and the polynomial bias
//       factors.
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::ComputePolynomialTextures()
{
    ...
    for( int i = 0; i < g_NumSourceImages; i++ )
    {
        ...
        hr = D3DXCreateTextureFromFileEx( m_pd3dDevice, g_ImageList[i].strFileName,
                                          D3DX_DEFAULT, D3DX_DEFAULT, 1, 0,
                                          D3DFMT_A8R8G8B8, 0,
                                          D3DX_FILTER_NONE, D3DX_FILTER_NONE,
                                          0, NULL, NULL, &pSourceTextures[i] );
    ...
    // Keep track of minimum and maximum of each coefficient.
    double MinCoeff[6];
    double MaxCoeff[6];
    ...
    for( int i = 0; i < 6; i++ )
    {
        // Scale is fixed at 2.0 because that is what our vertex shader uses.
        Scale[i] = 2.0f;
