...
XBHELP_CALLOUT g_HelpCallouts[] = 
{
    ...
    { XBHELP_B_BUTTON,     XBHELP_PLACEMENT_2, L"Toggle point\nlight" },
...
VOID CXBoxSample::DrawLight()
{
    ...
    D3DXVECTOR3 line[6];
    const D3DXVECTOR3 vXP( g_vPtLightPos.x +  0.2f, g_vPtLightPos.y +  0.0f, g_vPtLightPos.z +  0.0f );
    const D3DXVECTOR3 vXN( g_vPtLightPos.x + -0.2f, g_vPtLightPos.y +  0.0f, g_vPtLightPos.z +  0.0f );
    const D3DXVECTOR3 vYP( g_vPtLightPos.x +  0.0f, g_vPtLightPos.y +  0.2f, g_vPtLightPos.z +  0.0f );
    const D3DXVECTOR3 vYN( g_vPtLightPos.x +  0.0f, g_vPtLightPos.y + -0.2f, g_vPtLightPos.z +  0.0f );
    const D3DXVECTOR3 vZP( g_vPtLightPos.x +  0.0f, g_vPtLightPos.y +  0.0f, g_vPtLightPos.z +  0.2f );
    const D3DXVECTOR3 vZN( g_vPtLightPos.x +  0.0f, g_vPtLightPos.y +  0.0f, g_vPtLightPos.z + -0.2f );
    D3DXVec3TransformCoord( &line[0], &vXP, &matInvWorld );
    D3DXVec3TransformCoord( &line[1], &vXN, &matInvWorld );
    D3DXVec3TransformCoord( &line[2], &vYP, &matInvWorld );
    D3DXVec3TransformCoord( &line[3], &vYN, &matInvWorld );
    D3DXVec3TransformCoord( &line[4], &vZP, &matInvWorld );
    D3DXVec3TransformCoord( &line[5], &vZN, &matInvWorld );
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"PerPixelLighting" );
                            ...
                            CXBPackedResource* pResource )
{
    ...
    hr = XBUtil_CreateNormalizationCubeMap( 256, &m_pCubeMap );
    if( FAILED(hr) )
        return E_FAIL;
...
VOID CBumpyObject::CreateBasisMatrices()
{
    ...
    for( i=0; i < m_dwNumVertices; i++ )
    {
        ...
        {
            const D3DXVECTOR3 vDistance = pVertices[i].p - pVertices[j].p;
            FLOAT dist = D3DXVec3LengthSq( &vDistance );
                                    ...
                                    D3DXVECTOR3* pPtLightPos )
{
    ...
    const D3DXVECTOR3 vNegLightDir( -*pDirLightDir );
    D3DXVec3TransformCoord( &vDirLightWorldDir, &vNegLightDir, &matInvWorld );
