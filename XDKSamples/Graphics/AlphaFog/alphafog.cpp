...
HRESULT CXBoxSample::Initialize()
{
    ...
    // Load the terrain
    if( FAILED( m_TerrainObject.Create( "Models\\Terrain.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if( FAILED( XBUtil_CreateVertexShader( "Shaders\\AlphaFog.xvu",
                                           dwVertexDecl,
                                           &m_dwVertexShader ) ) )
        return E_FAIL;
...
HRESULT CXBoxSample::Render()
{
    ...
    m_TerrainObject.Render( XBMESH_NOFVF|XBMESH_NOTEXTURES );
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"AlphaFog" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        WCHAR strBuffer[80];
        swprintf( strBuffer, L"%0.2f", m_fFogStartValue );
        m_Font.DrawText(  64,  75, 0xffffffff,  L"Start:" );
        m_Font.DrawText( 150,  75, 0xffffff00, strBuffer  );
        swprintf( strBuffer, L"%0.2f", m_fFogEndValue );
        m_Font.DrawText(  64, 100, 0xffffffff,  L"End:" );
